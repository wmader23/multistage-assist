...
from custom_components.multistage_assist.conversation_utils import make_response, error_response, with_new_text, filter_candidates_by_state
...
class CommandProcessorCapability(Capability):
    ...
    async def process(self, user_input, candidates: List[str], intent_name: str, params: Dict[str, Any], learning_data=None, agent=None) -> Dict[str, Any]:
        ...
        if len(candidates) == 1:
            return await self._execute_final(user_input, candidates, intent_name, params, learning_data)
        ...
        if len(final_candidates) == 1:
            return await self._execute_final(user_input, final_candidates, intent_name, params, learning_data)
        ...
        return {
            "status": "handled", 
            "result": await make_response(msg_data.get("message", "Welches Gerät?"), user_input),
            "pending_data": {
                "type": "disambiguation",
    ...
    async def continue_disambiguation(self, user_input, pending_data: Dict[str, Any], agent=None) -> Dict[str, Any]:
    ...
    async def _execute_final(self, user_input, entity_ids, intent_name, params, learning_data=None):
        exec_data = await self.executor.run(user_input, intent_name=intent_name, entity_ids=entity_ids, params=params)
        ...
        # Confirmation
        speech = result_obj.response.speech.get("plain", {}).get("speech", "") if result_obj.response.speech else ""
        if not speech or speech.strip() == "Okay.":
            # Pass final_params instead of original params
            conf_data = await self.confirmation.run(user_input, intent_name=intent_name, entity_ids=entity_ids, params=final_params)
    ...
    async def execute_sequence(self, user_input, commands: List[str], agent) -> Dict[str, Any]:
        """Execute a list of atomic commands."""
        results = []
        for i, cmd in enumerate(commands):
            _LOGGER.debug("[CommandProcessor] Sequence %d/%d: %s", i+1, len(commands), cmd)
